<?php
/**
 * Booster Core for WooCommerce - Module - Product Price by Formula
 *
 * @version 1.0.0
 * @since  1.0.0
 * @author  Pluggabl LLC.
	...
	function change_price($price, $_product, $output_errors = false)
		{
			...
			if ($this->is_price_by_formula_product($_product) && '' != $price) {
				if ('' != wcj_get_option('wcj_product_cats_incl', '')) {
					if ('Include' == wcj_get_option('wcj_product_cats_option', '')) {
						$category_id = wcj_get_option('wcj_product_cats_incl', '');
						$_product_id_inc = $this->get_product_id_by_cat_id($category_id);
						$_product_id = wcj_get_product_id_or_variation_parent_id($_product);
						if (in_array($_product_id, $_product_id_inc)) {
							$saved_price = $this->get_saved_price($_product_id);
							if (false !== $saved_price) {
								return $saved_price;
							}
					...
					if ('Exclude' == wcj_get_option('wcj_product_cats_option', '')) {
