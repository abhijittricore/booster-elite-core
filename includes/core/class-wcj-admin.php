<?php
/**
 * Booster Core for WooCommerce - Core - Admin
 *
 * @version 1.0.0
 * @since  1.0.0
 * @author  Pluggabl LLC.
...
class WCJ_Admin {
	...
	function check_plus_version() {
		...
		foreach ( wcj_get_active_plugins() as $active_plugin ) {
			...
			if ( isset( $active_plugin[1] ) ) {
				if ( 'booster-core-for-woocommerce.php' === $active_plugin[1] ) {
					return;
				} elseif ( 'woocommerce-jetpack-plus.php' === $active_plugin[1] || 'woocommerce-booster-plus.php' === $active_plugin[1] ) {
					$is_deprecated_plus_active = true; // can't `brake` because of possible active `booster-core-for-woocommerce.php`
				}
			}
		}
		if ( $is_deprecated_plus_active ) {
			$class   = 'notice notice-error';
			$message = __( 'Please update <strong>Booster Core for WooCommerce</strong> plugin.', 'woocommerce-jetpack' ) . ' ' .
				sprintf(
				...
				sprintf(
					...
					'https://booster.io/booster-core-for-woocommerce-update/'
				);
	...
	function admin_footer_text( $footer_text ) {
		if ( isset( $_GET['page'] ) ) {
			if ( 'wcj-tools' === $_GET['page'] || ( 'wc-settings' === $_GET['page'] && isset( $_GET['tab'] ) && 'jetpack' === $_GET['tab'] ) ) {
				...
				return sprintf(
					__( 'If you like <strong>Booster Core for WooCommerce</strong> please leave us a %s rating. Thank you, we couldn\'t have done it without you!', 'woocommerce-jetpack' ),
					$rating_link
				);
			}
		}
	...
	function booster_menu() {
		add_submenu_page(
			'woocommerce',
			__( 'Booster Core for WooCommerce', 'woocommerce-jetpack' ),
