<?php
/**
 * Booster Core for WooCommerce - Functions - Admin
 *
 * @version 1.0.0
 * @since  1.0.0
 * @author  Pluggabl LLC.
...
if ( ! function_exists( 'wcj_admin_notices_version_updated' ) ) {
	...
	function wcj_admin_notices_version_updated() {
		if ( wcj_get_option( WCJ_VERSION_OPTION ) === WCJ()->version ) {
			...
			$message = sprintf( __( '<strong>Booster Core for WooCommerce</strong> plugin was successfully updated to version <strong>%s</strong>.', 'woocommerce-jetpack' ), WCJ()->version );
...
if ( ! function_exists( 'wcj_get_plus_message' ) ) {
	...
	function wcj_get_plus_message( $value, $message_type, $args = array() ) {
		...
		$module_current_section=apply_filters('wcj_current_section_get',$current_section);
		...
		switch ( $message_type ) {
			...
			case 'global':
				return '<div class="notice notice-warning">' .
						...
						'<p><span>' . sprintf( __( 'Some settings fields are locked and you will need %s to modify all locked fields.', 'woocommerce-jetpack'),
							'<a href="https://booster.io/plus/" target="_blank">Booster Core for WooCommerce Plus</a>' ) . '</span></p>' .
						'<p>' .
			...
			case 'desc':
				return sprintf( __( 'Get <a href="%s" target="_blank">Booster Addon</a> to change value.', 'woocommerce-jetpack' ), 'https://booster.io/plus/' );

			case 'desc_advanced':
				return sprintf( __( 'Get <a href="%s" target="_blank">Booster Addons</a> to enable "%s" option.', 'woocommerce-jetpack' ), 'https://booster.io/plus/', $args['option'] );

			case 'desc_advanced_no_link':
				return sprintf( __( 'Get Booster Addon to enable "%s" option.', 'woocommerce-jetpack' ), $args['option'] );

			case 'desc_below':
				return sprintf( __( 'Get <a href="%s" target="_blank">Booster Addon</a> to change values below.', 'woocommerce-jetpack' ), 'https://booster.io/plus/' );

			case 'desc_above':
				return sprintf( __( 'Get <a href="%s" target="_blank">Booster Addon</a> to change values above.', 'woocommerce-jetpack' ), 'https://booster.io/plus/' );

			case 'desc_no_link':
				return __( 'Get Booster Addon to change value.', 'woocommerce-jetpack' );
