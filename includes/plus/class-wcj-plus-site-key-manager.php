<?php
/**
 * Booster Core for WooCommerce - Plus - Site Key Manager
 *
 * @version 1.0.0
 * @since  1.0.0
 * @author  Pluggabl LLC.
...
class WCJ_Plus_Site_Key_Manager {
	...
	function __construct() {
		...
		$this->update_checker = new PluginUpdateChecker_3_1( $this->update_server_url . '/?alg_update_action=get_metadata&alg_update_slug=booster-core-for-woocommerce', WCJ_PLUGIN_FILE );
	...
	function maybe_add_after_plugin_row_key_error_message( $plugin_file, $plugin_data, $status ) {
		if ( ! $this->is_site_key_valid() && false !== ( $site_key_status = $this->get_site_key_status_message() ) ) {
			echo '<tr class="plugin-update-tr active" id="booster-core-for-woocommerce-update-site-key" data-slug="booster-core-for-woocommerce" data-plugin="' . plugin_basename( WCJ_PLUGIN_FILE ) . '">' .
	...
	function check_site_key() {
		if ( '' != ( $site_key = $this->get_site_key() ) ) {
			$url = $this->update_server_url . '/?check_site_key=' . $site_key . '&item_slug=booster-core-for-woocommerce&site_url=' . $this->site_url;
