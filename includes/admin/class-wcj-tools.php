<?php
/**
 * Booster Core for WooCommerce Tools
 *
 * @version 1.0.0
 * @author  Pluggabl LLC.
 */
...
class WCJ_Tools {
	...
	function add_wcj_tools() {
		if ( apply_filters( 'wcj_can_create_admin_interface', true ) ) {
			add_submenu_page(
				'woocommerce',
				__( 'Booster Core for WooCommerce Tools', 'woocommerce-jetpack' ),
	...
	function create_tools_page() {
		...
		if ( 'dashboard' === $active_tab ) {
			$title = __( 'Booster Core for WooCommerce Tools - Dashboard', 'woocommerce-jetpack' );
			$desc  = __( 'This dashboard lets you check statuses and short descriptions of all available Booster Core for WooCommerce tools. Tools can be enabled through WooCommerce > Settings > Booster. Enabled tools will appear in the tabs menu above.', 'woocommerce-jetpack' );
			echo '<h3>' . $title . '</h3>';
			echo '<p>' .  $desc . '</p>';
