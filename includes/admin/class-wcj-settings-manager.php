<?php
/**
 * Booster Core for WooCommerce - Settings Manager - Import / Export / Reset Booster's settings
 *
 * @version 1.0.0
 * @since  1.0.0
 * @author  Pluggabl LLC.
...
class WCJ_Settings_Manager {
	...
	function manage_options_import() {
		...
		if( ! isset( $_FILES['booster_import_settings_file']['tmp_name'] ) || '' == $_FILES['booster_import_settings_file']['tmp_name'] ) {
			...
			if ( ! is_array( $import_settings ) || 2 !== count( $import_settings ) ) {
				...
				$required_header = 'Booster Core for WooCommerce';
	...
	function manage_options_export() {
		...
		$export_settings = 'Booster Core for WooCommerce v' . wcj_get_option( WCJ_VERSION_OPTION, 'NA' ) . PHP_EOL . $export_settings;
